...
public class Eyes : MonoBehaviour, IExternalSensor
{
    ...
    void FixedUpdate()
    {
        ...
        Debug.DrawRay(transform.position, transform.up * visibleDistance, Color.red);
        Debug.DrawRay(transform.position, -transform.up * visibleDistance, Color.red);

        // Top facing raycast
        RaycastHit2D hit = Physics2D.Raycast(transform.position, transform.up, visibleDistance, 1 << LayerMask.NameToLayer("Limits"));
        //Debug.Log(hit.transform.gameObject.tag);
        if (hit.collider != null)
        {
            if (hit.collider.gameObject.tag == "top")
            {
                outputs[0] = (hit.transform.position.y - transform.position.y) / visibleDistance;
            }
        }
        // Bottom facing raycast
        hit = Physics2D.Raycast(transform.position, -transform.up, visibleDistance, 1 << LayerMask.NameToLayer("Limits"));
        //Debug.Log(hit.transform.gameObject.tag);
        if (hit.collider != null)
        {
            if (hit.collider.gameObject.tag == "bottom")
            {
                outputs[1] = (transform.position.y - hit.transform.position.y) / visibleDistance;
            }
        }
