...
public class Brain : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // Q data
        qs = SoftMax(ann.CalcOutput(states)); // SoftMax formats the output
        ...
        if(ball.GetComponent<BallState>().dropped)
        {
            ...
            for(int i = _replayMemory.Count - 1; i > 0; i--)
            {
                ...
                toutputsOld = SoftMax(ann.CalcOutput(_replayMemory[i].states)); // Compute Q values with current memory
                ...
                {
                    toutputsNew = SoftMax(ann.CalcOutput(_replayMemory[i + 1].states)); // Get the action from the next memory
